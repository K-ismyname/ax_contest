...
상단 토글 버튼(담당자 | 시민)으로 뷰를 전환한다.

### 담당자 뷰 (기본 랜딩)

**상단 경보 배너** — 경보 레벨·DD 현황·권고 문구를 한 줄로 표시 (색상 코딩).
...
**DD 경보 단계 바** — 관심 / 주의(현재) / 경보 세 구간을 progress bar로 시각화.

**자동 방제 권고 섹션** (주의 이상 시 노출)
- 권고 문구 (RAG 생성)
- 버튼: 이메일 발송 / 방제 체크리스트 / RAG 가이드

**하단 2단 레이아웃**
- 좌: 구별 위험도 순위 리스트 (높음/중간/낮음 태그)
- 우: 서울시 구별 위험도 히트맵 (Folium)

---

### 시민 뷰

**상단 경보 배지 + 대형 DD 숫자** — 현재 누적 DD / 임계값, 예상 첫 출현일 D-day.

**3개 행동 안내 카드** — 창문 방충망 점검 / 산·공원 인근 주의 / 제보 참여 독려.

**2단 레이아웃**
- 좌: 러브버그 발견 제보 폼 (발견 위치, 발견 일시, 사진 첨부, 제보하기 버튼)
- 우: 우리 동네 현황 — DD 진행률 바, 연도별 발생 이력(2023~2025), 올해 예측(첫 출현·최성기·종료)

**하단**: 최근 제보 현황 전체 목록 (실시간 갱신)

---

## 9. 구현 범위 외 (이번 스펙 제외)

- 사용자 인증 (시민/담당자 로그인) — 현재는 URL 파라미터로 구분
