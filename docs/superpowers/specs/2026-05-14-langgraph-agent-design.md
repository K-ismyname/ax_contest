...
단일 앱에서 사이드바 토글 또는 URL 파라미터(`?mode=official`)로 두 뷰를 전환한다.

### 시민 뷰 (`mode=citizen`, 기본)
- 현재 경보 레벨 배너 (색상 코딩)
- 제보 입력 폼 (날짜, 위치, 사진 선택)
- Folium 제보 지도 (최근 7일 제보 핀)
- 제보 후 RAG 대처법 안내 출력

### 담당자 뷰 (`mode=official`)
- 경보 레벨 + 현재 DD / 임계값 대비 게이지
- 연도별 DD 누적 추이 차트 (2022~현재)
- 서울 구별 제보 히트맵
- 오늘의 RAG 대응 요약 (주의 이상 시)
- 최근 14일 일별 DD 테이블
